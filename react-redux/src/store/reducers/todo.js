...
export default function todoReducer (state = initialState, { type, payload }) {
    switch (type) {
        ...
        case TODO_CHECK_ACTION:
            const editTodo = state.editTodo.done === false ? {...state.editTodo, done: true} : {...state.editTodo, done: false};
            return { ...state, list: state.list.map(todoItem => todoItem.id === editTodo.id ? editTodo : todoItem)} ;

            // if (state.editTodo.done === true) return { ...state, list: [...state.list, payload] }
            // else return { ...state, list: [...state.list, payload] }
